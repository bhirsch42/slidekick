...
function fixedHeight(type: string): number | null {
  if (type === "title") return TITLE_H;
  if (type === "subtitle") return SUBTITLE_H;
  if (type === "heading") return HEADING_H;
  return null;
}
...
function layoutNode(node: Node, area: Box, out: Placed[]): void {
  switch (node.type) {
    ...
    case "image":
      ...
      return;
    case "quote": {
      const attribution = node.props.attribution as string | undefined;
    ...
    case "columns": {
      ...
      const totalWeight = cols.reduce((s, c) => s + ((c.props.weight as number) ?? 1), 0);
      const gap = (node.props.gap as number) ?? 0.3;
