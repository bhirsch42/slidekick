...
import { layoutDeck, type Placed, type TextRole } from "./layout.js";

const PptxCtor = pptxgen as unknown as new () => any;

export async function renderPptx(slides: Node[], outPath: string): Promise<void> {
...
function drawPlaced(slide: any, p: Placed): void {
  if (p.kind === "image") {
    slide.addImage({
      path: p.src,
      x: p.x,
...
function textStyle(role: TextRole): Record<string, unknown> {
  switch (role) {
    case "title":
